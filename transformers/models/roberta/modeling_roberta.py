...
class RobertaModel(BertModelAdaptersMixin, RobertaPreTrainedModel):
    ...
    def forward(
        ...
        sample_space_tensor: Optional[torch.Tensor] = None
        ...
        # if sample_space_tensor is not None:
            # sequence_output_cls = sequence_output[:, 0, :]

            # sample_space_tensor_normalized = torch.nn.functional.normalize(sample_space_tensor, p=2, dim=1)
            # sequence_output_cls_normalized = torch.nn.functional.normalize(sequence_output_cls, p=2, dim=1)

            # cosine_similarities = torch.mm(sequence_output_cls_normalized, sample_space_tensor_normalized.T)

            # max_similarities, max_similarity_indices = torch.max(cosine_similarities, dim=1)
            
            # similarity_threshold = 0.5
            # below_threshold_indices = torch.where(max_similarities < similarity_threshold)[0]
            # print(len(below_threshold_indices), sequence_output.shape[0])

            # sequence_output_cls_proj = sequence_output_cls
            # for idx in below_threshold_indices:
            #     most_similar_embedding = sample_space_tensor[max_similarity_indices[idx]]
            #     sequence_output_cls_proj = torch.cat([sequence_output_cls[:idx], most_similar_embedding.unsqueeze(0), sequence_output_cls[idx+1:]], dim=0)

            # sequence_output = torch.cat([sequence_output_cls_proj.unsqueeze(1), sequence_output[:, 1:, :]], dim=1)
